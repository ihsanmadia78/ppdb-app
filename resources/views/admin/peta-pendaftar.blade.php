...
@section('title', 'Peta Lokasi Pendaftar')
...
<div class="container-fluid px-4 py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Visualisasi sebaran lokasi pendaftar PPDB ({{ $pendaftar->count() }} lokasi)</p>
        </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <ul class="list-unstyled mb-0">
                        <li><i class="fas fa-map-marker-alt text-primary me-2"></i>Dengan Koordinat: <strong>{{ $pendaftar->count() }}</strong></li>
                        <li><i class="fas fa-users text-secondary me-2"></i>Total Pendaftar: <strong>{{ \App\Models\Pendaftar::count() }}</strong></li>
                        <li><i class="fas fa-users text-success me-2"></i>Lulus: <strong>{{ \App\Models\Pendaftar::where('status', 'LULUS')->count() }}</strong></li>
                        <li><i class="fas fa-clock text-warning me-2"></i>Proses: <strong>{{ \App\Models\Pendaftar::whereIn('status', ['SUBMIT', 'VERIFIKASI_BERKAS', 'MENUNGGU_PEMBAYARAN', 'SIAP_SELEKSI'])->count() }}</strong></li>
                        <li><i class="fas fa-times text-danger me-2"></i>Tidak Lulus: <strong>{{ \App\Models\Pendaftar::where('status', 'TIDAK_LULUS')->count() }}</strong></li>
                        <li><i class="fas fa-list text-info me-2"></i>Cadangan: <strong>{{ \App\Models\Pendaftar::where('status', 'CADANGAN')->count() }}</strong></li>
                    </ul>
                </div>
                <div class="col-md-6">
...
<script>
    ...
    const markers = [];
    const pendaftarData = {!! json_encode($pendaftar->map(function($p) {
