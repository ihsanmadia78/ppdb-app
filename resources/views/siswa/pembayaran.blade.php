...
<div class="container-fluid">
    ...
    <div class="row">
            ...
            <div class="card h-100">
                ...
                <div class="card-body">
                    <div class="info-item mb-3">
                        ...
                        <p class="fw-bold mb-0">{{ $pendaftar->no_pendaftaran }}</p>
                    </div>
                    <div class="info-item mb-3">
                        <label class="text-muted small">Nama Lengkap</label>
                        <p class="fw-bold mb-0">{{ $pendaftar->dataSiswa->nama ?? 'Belum diisi' }}</p>
                    </div>
                    <div class="info-item mb-3">
                        <label class="text-muted small">Jurusan Pilihan</label>
                        <p class="fw-bold mb-0">{{ $pendaftar->jurusan->nama }}</p>
                    </div>
                    <div class="info-item mb-4">
                        <label class="text-muted small">Gelombang</label>
                        <p class="fw-bold mb-0">{{ $pendaftar->gelombang->nama }}</p>
                    </div>
                    ...
                    <div class="payment-amount text-center p-3 bg-primary bg-opacity-10 rounded">
                        ...
                        <h4 class="text-primary mb-0">Rp {{ number_format($pendaftar->gelombang->biaya_daftar ?? 150000, 0, ',', '.') }}</h4>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-8">
                                        ...
                                        <div class="qr-details mt-3">
                                            <div class="bg-light p-3 rounded">
                                                <div class="qr-code-container position-relative" onclick="openQRModal()">
                                                    {!! QrCode::size(100)->generate('SMK_BAKTINUSANTARA_666_PPDB_' . $pendaftar->no_pendaftaran . '_' . ($pendaftar->gelombang->biaya_daftar ?? 150000)) !!}
                                    ...
                                    <div class="card-body text-center">
                                        ...
                                        <div class="va-details mt-3">
                                            <div class="bg-light p-3 rounded">
                                                ...
                                                <h6 class="text-warning">8808{{ str_pad($pendaftar->id, 6, '0', STR_PAD_LEFT) }}</h6>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            @if($pendaftar->pembayaran)
                ...
                <div class="card mb-4 border-{{ $config['color'] }}">
                    <div class="card-header bg-{{ $config['color'] }} bg-opacity-10">
                        <h6 class="mb-0 text-{{ $config['color'] }}"><i class="fas fa-{{ $config['icon'] }} me-2"></i>Status Pembayaran Anda</h6>
                    </div>
                    <div class="card-body">
                        <div class="row align-items-center">
                            ...
                            <div class="col-md-9">
                                <div class="row">
                                    <div class="col-sm-6">
                                        <div class="info-item mb-2">
                                            ...
                                            <p class="fw-bold text-{{ $config['color'] }} mb-0">Rp {{ number_format($pendaftar->pembayaran->nominal, 0, ',', '.') }}</p>
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="info-item mb-2">
                                            <label class="text-muted small">Metode Pembayaran</label>
                                            <p class="fw-bold mb-0">{{ strtoupper($pendaftar->pembayaran->metode_pembayaran) }}</p>
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="info-item mb-2">
                                            <label class="text-muted small">Tanggal Upload</label>
                                            <p class="fw-bold mb-0">{{ $pendaftar->pembayaran->tanggal_bayar->format('d M Y, H:i') }}</p>
                                        </div>
                                    </div>
                                    @if($pendaftar->pembayaran->verified_at)
                                        <div class="col-sm-6">
                                            <div class="info-item mb-2">
                                                ...
                                                <p class="fw-bold mb-0">{{ $pendaftar->pembayaran->verified_at->format('d M Y, H:i') }}</p>
                                            </div>
                                        </div>
                                    @endif
                                    <div class="col-sm-6">
                                        @if($pendaftar->pembayaran->bukti_bayar)
                                            <div class="info-item">
                                                ...
                                                <div class="d-flex gap-2">
                                                    <a href="{{ Storage::url($pendaftar->pembayaran->bukti_bayar) }}" target="_blank" class="btn btn-outline-primary btn-sm">
                                                        <i class="fas fa-eye me-1"></i>Lihat Bukti
                                                    </a>
                                                    @if($status == 'verified')
                                                        <a href="{{ route('siswa.cetak-bukti-pembayaran') }}" class="btn btn-outline-success btn-sm">
                                ...
                                @if($pendaftar->pembayaran->catatan_verifikasi)
                                    <div class="row mt-3">
                                        <div class="col-12">
                                            <div class="alert alert-{{ $status == 'rejected' ? 'danger' : 'info' }} mb-0">
                                                <h6 class="alert-heading mb-2">
                                                    <i class="fas fa-{{ $status == 'rejected' ? 'exclamation-triangle' : 'info-circle' }} me-2"></i>
                                                    Catatan dari Tim Keuangan:
                                                </h6>
                                                <p class="mb-0">{{ $pendaftar->pembayaran->catatan_verifikasi }}</p>
            ...
            <!-- Upload Form -->
            @if(!$pendaftar->pembayaran || $pendaftar->pembayaran->status == 'rejected')
                <div class="card mb-4">
                    <div class="card-header bg-light">
                        <h6 class="mb-0">
                            <i class="fas fa-upload me-2"></i>
                            {{ $pendaftar->pembayaran && $pendaftar->pembayaran->status == 'rejected' ? 'Upload Ulang Bukti Pembayaran' : 'Upload Bukti Pembayaran' }}
                        </h6>
                    </div>
                    <div class="card-body">
                        @if($pendaftar->pembayaran && $pendaftar->pembayaran->status == 'rejected')
                        ...
                        <form action="{{ route('siswa.upload-pembayaran') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                                ...
                                <div class="col-12">
                                    <div class="d-grid d-md-flex justify-content-md-end">
                                        <button type="submit" class="btn btn-primary btn-lg px-4">
                                            <i class="fas fa-upload me-2"></i>
                                            {{ $pendaftar->pembayaran && $pendaftar->pembayaran->status == 'rejected' ? 'Upload Ulang' : 'Upload Bukti Pembayaran' }}
                                        </button>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="card mb-4 border-info">
                    ...
                    <div class="card-body text-center">
                        ...
                        <p class="text-muted mb-0">
                            @if($pendaftar->pembayaran->status == 'paid')
        ...
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <div class="qr-large mb-3">
                    {!! QrCode::size(250)->generate('SMK_BAKTINUSANTARA_666_PPDB_' . $pendaftar->no_pendaftaran . '_' . ($pendaftar->gelombang->biaya_daftar ?? 150000)) !!}
                </div>
                <div class="qr-info">
                    <h6 class="text-primary">Informasi Pembayaran</h6>
                    <p class="mb-1"><strong>No. Pendaftaran:</strong> {{ $pendaftar->no_pendaftaran }}</p>
                    <p class="mb-1"><strong>Nama:</strong> {{ $pendaftar->dataSiswa->nama ?? 'Belum diisi' }}</p>
                    <p class="mb-1"><strong>Jurusan:</strong> {{ $pendaftar->jurusan->nama }}</p>
                    <p class="mb-3"><strong>Total Bayar:</strong> <span class="text-success fw-bold">Rp {{ number_format($pendaftar->gelombang->biaya_daftar ?? 150000, 0, ',', '.') }}</span></p>
...
function downloadQR() {
    ...
    img.onload = function() {
        ...
        link.download = 'qr-code-pembayaran-{{ $pendaftar->no_pendaftaran }}.png';
